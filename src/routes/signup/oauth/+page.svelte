<script lang="ts">
  ...
  onMount(() => {
    // URL에서 token과 provider 읽기
    const params = new URLSearchParams(window.location.search);
    signupToken = params.get('token');
  ...
  function validateNicknameLocally() {
    ...
    if (trimmed.length === 0) {
      ...
      isNicknameValidLocal = false; return;
    }
    if (trimmed.length < minLength) {
      nicknameMessage = `닉네임은 최소 ${minLength}글자 이상이어야 합니다.`; isNicknameValidLocal = false; return;
    }
    if (trimmed.length > maxLength) {
      nicknameMessage = `닉네임은 최대 ${maxLength}글자를 초과할 수 없습니다.`; isNicknameValidLocal = false; return;
    }
    if (!nicknameRegex.test(trimmed)) {
      nicknameMessage = "닉네임에는 영문, 숫자, 밑줄(_)만 사용할 수 있습니다."; isNicknameValidLocal = false; return;
    }
    if (trimmed.includes("__")) {
      nicknameMessage = "닉네임에 밑줄(_)을 연속으로 사용할 수 없습니다."; isNicknameValidLocal = false; return;
    }

    const reservedUsernames = ["admin","root","system","support","testuser"];
    if (reservedUsernames.includes(trimmed.toLowerCase())) {
      nicknameMessage = "해당 닉네임은 예약어로 사용할 수 없습니다."; isNicknameValidLocal = false; return;
    }
  ...
  async function checkNicknameAvailability() {
    ...
    try {
      const res = await fetch(`/api/account/availability?field=nickname&value=${encodeURIComponent(nickname.trim())}`);
  ...
  async function handleSubmit() {
    ...
    if (!isNicknameValidLocal || !isNicknameChecked || !isNicknameAvailableServer) {
      alert("닉네임 유효성 및 중복 확인을 완료해 주세요.");
      return;
    }

    if (!signupToken || !provider) {
      formErrorMessage = "OAuth 정보가 누락되었습니다. 다시 시도해 주세요.";
      return;
    }
...
<div class={styles.container}>
  <div class={styles.contentWrapper}>
    <div class={styles.logoSection}>
      <div class={styles.logoCircle}><span class={styles.logoEmoji}>🪐</span></div>
    ...
    <div class={styles.signupCard}>
      ...
      <p class={styles.intro}>오늘의 할 일을 기록하고,<br/>목표를 달성하며 친구들과 공유하세요 🌟</p>

      <form on:submit|preventDefault={handleSubmit}>
        <div class={styles.profileSection}>
          <div class={styles.preview} on:click={triggerFileInput}>
            {#if previewUrl}<img src={previewUrl} alt="프로필 미리보기" />
            {:else}<span class={styles.placeholderText}>🌌</span>{/if}
          </div>
        ...
        <div class={styles.formGroup}>
          ...
          <div class={styles.nicknameCheck}>
            <input 
              id="nickname" type="text" bind:value={nickname} placeholder="영문, 숫자, 밑줄 3~15글자"
              class={styles.inputField} maxlength={maxLength} />
            <button 
              type="button" class={styles.checkBtn} 
              on:click={checkNicknameAvailability}
              disabled={!isNicknameValidLocal || isNicknameChecked && isNicknameAvailableServer}>
              {#if isNicknameChecked && isNicknameAvailableServer} 확인 완료 {:else} 중복 확인 {/if}
            </button>
          </div>
          {#if nicknameMessage}
            <p class={`${styles.nicknameMessage} ${isNicknameAvailableServer && isNicknameChecked ? styles.valid : ''} ${(!isNicknameAvailableServer && isNicknameChecked) || !isNicknameValidLocal ? styles.error : ''}`}>
              {nicknameMessage}
            </p>
          {/if}
        </div>
        ...
        <div class={styles.formGroup}>
          ...
          <textarea id="bio" bind:value={bio} rows="3" placeholder="간단한 자기소개를 입력해주세요 (최대 100자)" maxlength="100" class={styles.textareaField}></textarea>
        </div>
        ...
        <button type="submit" class={styles.submitBtn} disabled={!isNicknameAvailableServer || !isNicknameChecked || !isNicknameValidLocal || isSubmitting}>
          {#if isSubmitting}가입 중... 🚀{:else}가입하기 🚀{/if}
        </button>
      </form>
    </div>
